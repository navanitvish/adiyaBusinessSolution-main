    ...
    <div className={`container mx-auto w-10/12 px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 ${isLightMode ? 'text-lightText' : 'text-darkModeText'}`}>
      <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
        <div className="w-full lg:w-1/2 lg:pr-8" data-aos="fade-right">
          ...
          <p className={`mb-6 sm:mb-8 font-normal text-base sm:text-lg lg:text-xl ${isLightMode ? 'text-darkText' : 'text-darkSubHeading'}`}>
          ...
          <button
            ...
            className={`w-full sm:w-auto border px-4 sm:px-5 py-2 sm:py-2.5 rounded-md text-lg sm:text-xl font-normal transition-all duration-300 ease-in-out hover:-translate-y-1 hover:scale-105 ${
              isLightMode ? 'border-black bg-customBlue text-white hover:bg-customBlueHover hover:text-white' : 'border-white bg-customBlue text-white hover:bg-customBlueHover'
            }`}
          >
            Know more about us
          </button>
          ...
          <div className="grid grid-cols-2 gap-4 text-center mt-8">
            {aboutContent.stats.map((stat, index) => (
              <div key={index} className={`border py-2 sm:py-6 ${isLightMode ? 'border-black' : 'border-white'}`}>
                <h4 className={`text-2xl sm:text-3xl lg:text-4xl font-bold ${isLightMode ? 'text-darkText' : 'text-darkModeText'}`}>{stat.value}</h4>
                <p className={`text-sm sm:text-base lg:text-lg font-normal ${isLightMode ? 'text-blue-800' : 'text-gray-300'}`}>{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
